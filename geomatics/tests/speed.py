...
def test_engine(files: list, var: str or int, coords: tuple, min_coords, max_coords, dims: tuple, engine: str,
    ...
    geomatics.timeseries.point(files, var, coords, dims, engine=engine, xr_kwargs=xr_kwargs, strp_filename=strp_filename)
    t2 = datetime.datetime.now()
    geomatics.timeseries.bounding_box(files, var, min_coords, max_coords, dims, engine=engine, xr_kwargs=xr_kwargs, strp_filename=strp_filename)
    t3 = datetime.datetime.now()
    geomatics.timeseries.polygons(files, var, poly, dims, engine=engine, xr_kwargs=xr_kwargs, strp_filename=strp_filename)
